// the most hacky 3DS file loader class
#include "stdafx.h"
...
void Model_3DS::Load(char *name)
{
	...
	if(!glLockArraysEXT) FatalError("Cant load extension %s\n", "glLockArraysEXT");
	glUnlockArraysEXT	= (PFNGLUNLOCKARRAYSEXTPROC)	wglGetProcAddress("glUnlockArraysEXT");
	if(!glUnlockArraysEXT) FatalError("Cant load extension %s\n", "glUnlockArraysEXT");
	...
	// Load the file
	if((bin3ds = fopen(name,"rb")) == NULL)
		FatalError("cant open %s!", name);		
	LogLine("ESModel3DS: %s\n", name);
	...
	while (ftell(bin3ds) < (findex + length - 6))
	{
		...
		switch (h.id)
		{
			case TRIG_MESH	:
				...
				break;
			case OBJ_LIGHT	:
				LightChunkProcessor(h.len, ftell(bin3ds), objindex);
			default			:
				break;
		}
...
void Model_3DS::LightChunkProcessor(long length, long findex, int objindex)
{
	ChunkHeader h;
	CVector3 lightpos;
...
void Model_3DS::FacesMaterialsListChunkProcessor(long length, long findex, int objindex, int subfacesindex)
{
	...
	// Read the faces into the array
	for (auto i = 0; i < numEntries * 3; i+=3)
	{
		// read the face
