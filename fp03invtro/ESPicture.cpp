...
void CPicture::Create(float x, float y, float z, float width, float height, int face_filepath, int mask_filepath)
{
	x = x - width  / 2;
	y = y - height / 2;

	face.LoadBMPResource(face_filepath);
	if(mask_filepath) {
		LogLine("face: %d\n", face_filepath);
		LogLine("mask: %d\n", mask_filepath);
		b_Masked = true;
		mask.LoadBMPResource(mask_filepath);
	}
...
//	glLoadIdentity();
	...
	glBegin(GL_QUADS);		
		...
		glTexCoord2i(0.0f, 1.0f); glVertex3f(x,			y + height, z);
		glTexCoord2i(1.0f, 1.0f); glVertex3f(x + width, y + height, z); 
		glTexCoord2i(1.0f, 0.0f); glVertex3f(x + width, y,			z);
	glEnd();
	glEndList();
}
