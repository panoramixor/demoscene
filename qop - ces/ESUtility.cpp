...
void esDrawFps(float sTime)
{
	char fontname[]="./data/fonts/font.bf";
	static BFL_FONT  *bflfont=0;
	if(!bflfont)
		bflfont = BFL_LoadFont((FILE *)fontname);

	char strfps[100];
	char strtime[100];
	float fps = esGetFps(sTime);
	sprintf(strfps, "fps:%f", fps);
	sprintf(strtime, "time:%f", fTime);
	...
	glPushMatrix();
		glLoadIdentity();
		glTranslatef(0.7f, 0.65f, -1);
		glScalef(0.001f, 0.001f, 0.001f);
		glColor4f(1,1,1,1);
		BFL_DrawString(bflfont, strfps, (int)strlen(strfps));
	glPopMatrix();
	...
	glPushMatrix();
		...
		glScalef(0.001f, 0.001f, 0.001f);
		glColor4f(1,1,1,1);
		BFL_DrawString(bflfont, strtime, (int)strlen(strtime));
	glPopMatrix();
...
{	
	...
	SetDeviceGammaRamp(screen,(LPVOID)ramp);
...
{
/*
	float t1 = timepassed * (timepassed - 0.5f) * (timepassed - 1) + timepassed;
*/
	float t1;
	if(timepassed >= 0.5f) {
